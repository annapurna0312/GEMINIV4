        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                //if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                    String strength = "";
                    if(rssi<-90){ strength = "Unusable"; }
                    if(rssi>=-90 && rssi<-80){ strength = "Not Good"; }
                    if(rssi>=-80 && rssi<-70){ strength = "Okay"; }
                    if(rssi>=-70 && rssi<-67){ strength = "Very Good"; }
                    if(rssi>=-67 && rssi<-30){ strength = "Amazing"; }
                    mNewDevicesArrayAdapter.add(device.getName()+"["+rssi+"]("+strength+")" + "\n" + device.getAddress());
